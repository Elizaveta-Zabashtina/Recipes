...
class RealmManager {
    ...
    private func getRealm() -> Realm {
        if NSClassFromString("XCTest") != nil {
            return try! Realm(configuration: Realm.Configuration(
                fileURL: nil, inMemoryIdentifier: "test",
                encryptionKey: nil, readOnly: false,
                schemaVersion: 0, migrationBlock: nil, objectTypes: nil))
        } else {
            return try! Realm()
        }
    }
    ...
    func isAddedObject(_ recipe: Recipe) -> Bool {
        let realm = getRealm()
        realm.refresh()
        let recipes = realm.objects(Recipe.self).filter("id == \(recipe.id)")
